...
describe('ChangeTodoStatusUseCase', () => {
  ...
  it('存在しないTodoの場合はエラーを返す', async () => {
    ...
    await expect(useCase.execute({ id: 'non-existent-id', action: 'complete' }))
  ...
  it('既に完了済みのTodoを完了しようとするとエラーを返す', async () => {
    ...
    await expect(useCase.execute({ id: todo.id, action: 'complete' }))
  ...
  it('既にキャンセル済みのTodoをキャンセルしようとするとエラーを返す', async () => {
    ...
    await expect(useCase.execute({ id: todo.id, action: 'cancel' }))
      .rejects.toThrow('Todo is already cancelled');
  });
}); 